...
/* ---------------- 提示詞（只在這裡，網頁不能改） ---------------- */
/* 兩個提示詞共用的文體規範。
   模型讀的是英文摘要、寫的是中文，若不特別交代，它會連句法一起搬過來。
   注意這裡要的是「正式書面語＋中文句法」，不是白話——讀者是醫師，這份整理是臨床決策的起點。
   所以下面每一組前後對照的兩邊都是正式語體，差別只在句法。 */
const STYLE = `文體：
- 語體：專業、書面、精確，比照台灣醫學期刊的中文摘要或醫院實證中心的證據摘要。讀者是醫師。
- 不要口語化。不可出現「差很多」「拖比較久」「看起來有用」「花費也不高」「早點篩出」這類說話語氣。
- 也不要翻譯腔。你看到的摘要是英文的，但你不是在翻譯——翻譯腔的毛病在於句法是英文的，
  不在於用詞正式。要換掉的是句法，不是把格調降下來。下面每一組的左右兩邊都是正式中文，只有句法不同：
  「術後譫妄在髖部骨折高齡患者中發生率極高」→「高齡髖部骨折病人的術後譫妄發生率高」
  「文獻在預防措施的效果上存在異質性」→「各研究對預防措施的效果報告不一，異質性高」
  「多項研究顯示非藥物介入具有潛力」→「多項研究支持非藥物介入的效果」
  「目前仍處於機制探討與初步臨床觀察階段」→「目前仍限於機制探討與初步臨床觀察」
  「實務上常受限於人力培訓不足、資源匱乏及缺乏標準化流程」→「實務上受限於人力、訓練與標準化流程不足」
- 具體要避開的句式：
  「在 X 患者中」這種由 in…patients 直譯而來的框架，直接寫「X 病人的…」。
  「存在＋抽象名詞」，改用形容詞或動詞：「存在異質性」→「異質性高」；「存在爭議」→「仍有爭議」。
  「透過 A 進行 B」「予以」「加以」這類冗贅動詞，保留其一即可。
  一句話超過四十字就斷句；同一句不要出現三個以上的「的」。
- 台灣醫界用語，不可使用大陸慣用詞：
  干預→介入　圍術期／圍手術期→周術期　養護機構→長照機構　篩查→篩檢　隨訪→追蹤
  隊列研究→世代研究　薈萃分析→統合分析　依從性→遵從性　患病率→盛行率
  置信區間→信賴區間　比值比→勝算比　質量→品質　信息→資訊　概率→機率
- 「病人」優先於「患者」，同一篇不要混用。
- 上面交代給你的分類字眼（例如「證據直接支持」「延伸推論」）是寫作指示，不是要你當標題抄進文章裡。`;
...
對這個問題的意涵：從上述證據能推到的實務要點。哪些是文獻直接測過的、哪些是你從機轉或旁證推的，要讓讀者分得出來，但用句子講清楚，不要貼標籤——例如「這三篇都直接比過」「不過這是從機轉推的，還沒有人真的試」。若證據不足以回答，就直說。
規則：
...
合格的寫法大概長這樣（示範的是語體與句子的長相，內容不要照抄）：
共識：多數研究一致指出，高齡髖部骨折病人的術後譫妄發生率高，且與認知功能長期下降、住院天數延長及後續入住長照機構相關 [3][6]。預防方面，非藥物介入的證據較為一致：周術期音樂介入可降低發生率，並具成本效益 [1]；護理師主導的多成分照護亦呈現相同方向的效果 [10]。高齡、衰弱、營養不良、術前認知功能不佳與貧血，均為獨立的風險因子 [5][8]。`;
